...
def analyze_tds_voltage_and_energy(df_al, df_c):
    ...
    Main function for TDS-voltage correlation and total energy analysis
    ...
    # ── TDS-전압 상관관계 ── / TDS-Voltage Correlation
    # 흑연: TDS와 전압이 모두 있는 행 / Graphite: rows with both TDS and voltage
    c_tds = df_c.dropna(subset=["tds_ppm", "voltage_mV"])[["tds_ppm", "voltage_mV", "elapsed_hours"]].copy()
    # 알루미늄: 유효 행 (이상치 제외) / Aluminum: valid rows (excluding anomalies)
    ...
    # TDS-전압 산점도 / TDS-Voltage scatter plot
    ...
    # ── 사다리꼴 적분법을 이용한 총 에너지 ── / Total energy using trapezoidal integration
    def total_energy(df, group_label):
        # df를 elapsed_hours 기준으로 정렬 / Sort df by elapsed_hours
        ...
        # trapz: if t is in seconds, ∫P dt = J, then /3600 → Wh
    ...
    # 전력 적분 그림 / Power integration plot
    fig, axes = plt.subplots(2, 2, figsize=(14, 10))

    # 흑연: 전력 대 시간 / Graphite: Power vs time
    ...
    # 흑연: 누적 발전량 / Graphite: Cumulative energy
    ...
    # 알루미늄: 전력 대 시간 / Aluminum: Power vs time
    ...
    # 알루미늄: 누적 발전량 / Aluminum: Cumulative energy
